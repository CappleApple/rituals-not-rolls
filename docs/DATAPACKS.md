...
When several known entries match the same material, the maximum power wins; affinities never stack on that material. One unconsumed pedestal per actual item ID participates per enchantment; further unconsumed copies are excluded from the chain. Consumption-marked duplicates each contribute and each consume one item. Components do not change item identity. Each built-in pedestal accepts exactly one material. A tag discovery remains that tag discovery when tag membership changes.
...
Override `data/ritualsnotrolls/ritual_enchanting/rules.json`:

```json
{
  "consumption_multiplier": 2,
  "xp_levels_per_catalyst": 10,
  "xp_bonus_per_catalyst": 0.1,
  "conflict_multipliers": { "damage": 2, "protection": 2, "mining": 2 },
  "duration_ticks": 120
}
```

For N displayed Experience Catalysts, the multiplier is `1 + N × xp_bonus_per_catalyst`. The cost basis is `L = N × xp_levels_per_catalyst`. Convert the combined L to the XP needed to reach that level from zero:
...
Defaults therefore charge 160 / 550 / 1395 / 2920 XP for 1 / 2 / 3 / 4 catalysts. Bonuses add; they do not compound. The entire cost is checked and paid once. Insufficient XP does not silently ignore some catalysts. The old `xp_curve_per_100_points`, `max_xp_points`, and `automation_levels` fields are no longer used.

A Consumption Catalyst in a pedestal’s attachment slot applies the multiplier only to that pedestal’s material, which is taken on particle arrival for successful chains and committed on completion. Interrupted reservations are refunded; failed chains leave offerings in place. Other pedestals receive no sacrifice multiplier. Consumption-marked duplicate pedestals contribute independently and all participating copies are consumed once. Experience Catalysts count only in displayed item slots. A Subtraction Catalyst in the second attachment slot reverses the material contribution, and can coexist with consumption. All catalysts remain intact.

Group multipliers define the exponential base: the nth member costs `base^(n-1)`. A referenced group absent from the rules defaults to base 2. Existing enchantments occupy positions first; compatible additions are considered in full registry-ID order. Unaffordable additions are skipped. The highest affordable configured level is applied automatically. The configured duration is a minimum of 20–12000 ticks; it extends when needed for all ring formations plus 24 ticks together and the 28-tick pulse. Enchantment channels start sequentially by default; set `sequentialEnchantmentAnimations = false` in the server config for simultaneous starts. The setting is captured when the ritual starts. Levels per XP catalyst are 1–1000, and the bonus is a finite nonnegative value up to 100.
...
For chest loot it replaces each configured enchantment on an enchanted book with one random affinity page. Unconfigured enchantments stay on the original book, preserving its other components. Non-book loot and non-chest loot remain unchanged. Chest contexts are recognized by a `chests/` loot-table path or a loot-container block entity at the context origin. The optional `chance` adds an extra random page; it defaults to 0 and does not disable conversion. To disable the modifier entirely, override the global loot modifier list. Other loot tables can create exact pages using `minecraft:set_components` and this component value:
...
The dynamic 3-leather/page recipe has type `ritualsnotrolls:knowledge_book`. Other recipes are ordinary 1.21.1 `recipe` JSON files. Both knowledge books and loose knowledge pages are accepted. Default chiseled-bookshelf acceptance is added through `data/minecraft/tags/item/bookshelf_books.json`.
...
The combined catalyst capacity still uses `xp_levels_per_catalyst` and costs the XP needed to reach that total level from zero. The payable amount is capped by the player's available XP. Convert that amount back to equivalent levels, including fractional progress to the next level; the bonus is `equivalent_levels / xp_levels_per_catalyst * xp_bonus_per_catalyst`. Thus the defaults give +15% for 315 XP with two catalysts. Zero available XP contributes no bonus or XP particles and does not block an otherwise viable ritual. Menu previews, allocation, failure previews, payment and XP ring size all use the same budget.
