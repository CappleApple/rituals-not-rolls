...
| `menu` / `network` | Server menus, sequenced context-checked actions, definition and read-only reference synchronization |
...
For a modded pedestal with another slot layout, expose an adapter handler mapping logical material slot 0 and auxiliary modifier slots onto that layout. Implement simulate, insertion, and extraction honestly. A bookshelf adapter may be read-only because knowledge is read from physical knowledge book/page components without extracting either item.
...
- `ItemEntity.playerTouch` suppresses auto-add only for explicitly torn-out overflow pages.
