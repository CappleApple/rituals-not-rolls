...
| `knowledge` | Immutable page/book/binder components, crafting, inventory collection and physical library transfers |
...
Knowledge Book Auto-Add uses these acquisition hooks rather than inventory polling. `Knowledge.absorb` is also a public integration entry point for alternate acquisition systems. Direct `setItem` writes deliberately do not trigger book Auto-Add: that method alone cannot distinguish intentional rearrangement, load, synchronization, and acquisition. Mods bypassing both vanilla add/menu paths and NeoForge item handlers should call the explicit integration method when they know the insertion context.
...
`BinderTransfers` captures the real dropped binder and meters page checks at `rules.binderPagesPerSecond` per 20 ticks. Each accepted page becomes a real item entity using the ordinary library-transfer path before its stored count is reduced. Unusable entries count against the batch budget and remain stored. Binding can spend nearby owner-thrown leather. The binder releases once all remaining entries have been attempted and the table's pending library transfers finish. Interruptions preserve stored contents; pages already submitted follow ordinary transfer recovery. These classes describe the current internal implementation, not a separate stable integration API.

## Transactions and multiplayer

Menus are server-authoritative and validate container ID, sequence number, reachability, and bound book or binder identity. Replaying a packet cannot tear the same entry twice. Book and binder screens have no inventory slots. The table accepts a sequenced `retrieve` action for an enchantment in its current filtered library; it never accepts client-supplied inventory positions or stacks. Book and binder actions bind to the same physical stack while its UI is open. Binder actions address entries by a server-validated index; clients cannot submit stored page stacks.

Table-side book binding reserves a destination slot and enchantment while three real leather items orbit a captured page for 40 ticks. Three leather are separated from dropped stacks before capture. Library books are checked at capture, binding completion and shelf arrival; loose shelf pages do not count as existing books. The new book uses the page's knowledge component, matching the crafting recipe. New book placement is limited to native chiseled bookshelves; custom bookshelf adapters still participate in existing-book and duplicate detection.

Ingredients remain real item entities until the page changes into a book. Interrupted binding releases them intact; interruption after creation releases the completed book. Capture markers restore gravity and pickup on load, and claimed ingredients cannot merge during animation. Item-join callbacks are followed by stack revalidation before inputs change. All animation positions use the table's local space, including Sable plots.

An enchanting ritual has one active owner, determined by the dropped item's throwing player. Library transfers can run concurrently, including additions to the same book. Pending discoveries and first-book bindings reserve their enchantment/entry so overlapping submissions cannot create duplicates; arrivals re-read the destination and preserve earlier additions. Item capture also claims a specific live entity so overlapping tables cannot both use it. Sessions use a bounded lifetime and keep the target in the world rather than persisting an invisible copy. Reload revisions, disconnection, dimension changes, unloaded chunks, and missing tables invalidate sessions. Capture disables gravity and holds a server anchor in the table's coordinate space while the vanilla renderer supplies smooth bob and rotation. Completion and interruption restore the original gravity state. Persisted recovery markers also restore orphaned items when they rejoin the world after a crash.
