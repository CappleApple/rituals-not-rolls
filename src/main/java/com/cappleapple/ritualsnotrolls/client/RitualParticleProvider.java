...
public final class RitualParticleProvider implements ParticleProvider<RitualParticleOptions> {
  ...
  public Particle createParticle(
    ...
    return options.flight().isEmpty()
        ? particle
        : new Flying(level, new Vec3(x, y, z), particle, options, r, g, b);
  }
  ...
  private static final class Flying extends Particle {
    ...
    public void render(VertexConsumer buffer, Camera camera, float partial) {
      ...
      visual.render(
          flight.burst() || flight.instability().fails()
              ? new Fade(
                  buffer,
                  flight.burst()
  ...
  /** Modulate actual vertex opacity; atlas particles use blending during the burst. */
  private record Fade(VertexConsumer delegate, float opacity) implements VertexConsumer {
    @Override
